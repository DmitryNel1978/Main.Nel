...
class Main {
    ...
    public static String calc(String input) {
        ...
        if (parts.length != 3) { // проверяем, что выражение состоит из трех частей (двух чисел и оператора)
            return "throws Exception"; // если не соответствует формату, возвращаем сообщение 
        }
        try {
            ...
            return "throws Exception"; // если не удалось преобразовать, возвращаем сообщение
        }
        if (a < 1 || a > 10 || b < 1 || b > 10) { // проверяем, что числа находятся в диапазоне от 1 до 10
            return "throws Exception"; // если не находятся, возвращаем сообщение 
        }
        int result;
        switch (parts[1]) { // проверяем оператор
            ...
            case "/":
                if (b == 0) { // проверяем, что делитель не равен нулю
                    return "throws Exception"; // если равен, возвращаем сообщение
                }
                result = a / b;
                break;
            default:
